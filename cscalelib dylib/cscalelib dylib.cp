...
extern "C" GLuint make_rectangle_vbo(GLdouble vertices[4][2],double tex_w,double tex_h){ //tex_w and tex_h id the number of texture repeats over the width and height
	GLuint vbo;
	glGenBuffers(1,&vbo);
	glBindBuffer(GL_ARRAY_BUFFER, vbo);
	VertexData3D data[4];
	data[0].x = vertices[0][0];
	data[0].y = vertices[0][1];
	data[0].z = vertices[0][2];
	data[0].s = 0;
	data[0].t = 0;
	data[1].x = vertices[1][0];
	data[1].y = vertices[1][1];
	data[1].z = vertices[1][2];
	data[1].s = 0;
	data[1].t = tex_h;
	data[2].x = vertices[2][0];
	data[2].y = vertices[2][1];
	data[2].z = vertices[2][2];
	data[2].s = tex_w;
	data[2].t = tex_h;
	data[3].x = vertices[3][0];
	data[3].y = vertices[3][1];
	data[3].z = vertices[3][2];
	data[3].s = tex_w;
	data[3].t = 0;
	glBufferData(GL_ARRAY_BUFFER, sizeof(data), (GLvoid*)data, GL_STATIC_DRAW);
	return vbo;
}
extern "C" void draw_rect(GLuint vbo,GLuint texture){
	cout << glGetError() << "\n";
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S,GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T,GL_CLAMP_TO_EDGE);
	glColor4d(1,1,1,1);
	...
	glLoadIdentity();
	glBindBuffer(GL_ARRAY_BUFFER, vbo);
	//Make the pointers to vbo data. The stride is the size of 5 GLdouble for x,y,z,s,t
	glVertexPointer(3, GL_DOUBLE, sizeof(GLdouble)*5, (GLvoid*)offsetof(VertexData3D, x)); //3 ordinates per vertex starting at offset to attribute x in the VertexData3D struct
	glTexCoordPointer(3, GL_DOUBLE, sizeof(GLdouble)*5, (GLvoid*)offsetof(VertexData3D, s)); //3 ordinates per texture coordinate starting at offset to attribute s in the VertexData3D struct
	cout << glGetError() << "\n";
	glEnableClientState(GL_VERTEX_ARRAY);
	glEnableClientState(GL_TEXTURE_COORD_ARRAY);
	glDrawArrays(GL_TRIANGLES, 0, 3); //Draw as triangles as it is apparently faster.
	glDrawArrays(GL_TRIANGLES, 1, 3);
	glDisableClientState(GL_TEXTURE_COORD_ARRAY);
	glDisableClientState(GL_VERTEX_ARRAY);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
